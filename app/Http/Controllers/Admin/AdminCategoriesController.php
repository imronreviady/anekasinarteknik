...
class AdminCategoriesController extends Controller
{
	public function getCategories($language_id){
		...
		->select('categories.categories_id as id', 'categories.categories_image as image', 'categories.parent_id as parent_id', 'categories.date_added as date_added', 'categories.last_modified as last_modified', 'categories_description.categories_name as name', 'categories_description.language_id as language_id')
	...
	public function getSubCategoryByParentId($language_id, $parent_id)
	{
		$language_id     =   $language_id;		
		$listing = DB::table('categories')
		->leftJoin('categories_description','categories_description.categories_id', '=', 'categories.categories_id')
		->select('categories.categories_id as id', 'categories.categories_image as image',  'categories.date_added as date_added', 'categories.last_modified as last_modified', 'categories_description.categories_name as name')
		->where('categories_description.language_id','=', $language_id )->where('parent_id','>', '0')->where('parent_id','=', $parent_id)->get();
		return($listing);
	}
